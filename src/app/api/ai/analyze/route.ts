...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!transcript || typeof transcript !== 'string') {
      return NextResponse.json({ error: 'Valid "transcript" string is required' }, { status: 400 });
    }
    ...
    const prompt = `
Analyze the following meeting transcript and return a JSON object containing:
...
${transcript}`;
    ...
    return NextResponse.json({ error: 'Failed to analyze meeting artifacts.' }, { status: 500 });
  }
}
